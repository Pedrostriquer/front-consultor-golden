...
export const Logo = (props) => {
  return (
    <svg viewBox="0 0 293 368" xmlns="http://www.w3.org/2000/svg" {...props}>
      {/* NOVO: Definição do filtro de brilho MODIFICADO */}
      <defs>
        <filter id="glow" x="-50%" y="-50%" width="4000%" height="4000%"> {/* Aumenta a área de atuação do filtro */}
          {/* PRIMEIRO BRILHO (mais distante e suave) */}
          <feGaussianBlur in="SourceGraphic" stdDeviation="2" result="blur1" /> {/* Aumentado para 8 */}
          <feFlood floodColor="#f6d168" floodOpacity="0.8" result="color1" /> {/* Cor do brilho com opacidade */}
          <feComposite in="color1" in2="blur1" operator="in" result="glow1" />
          
          {/* SEGUNDO BRILHO (mais próximo e intenso) */}
          <feGaussianBlur in="SourceGraphic" stdDeviation="4" result="blur2" /> {/* Reduzido para ser mais focado */}
          <feFlood floodColor="#f6d168" floodOpacity="1" result="color2" /> {/* Cor do brilho principal, opaco */}
          <feComposite in="color2" in2="blur2" operator="in" result="glow2" />

          {/* TERCEIRO BRILHO (Opcional: para um "core" mais forte) */}
          {/* Você pode comentar ou descomentar esta seção para experimentar */}
          <feGaussianBlur in="SourceGraphic" stdDeviation="2" result="blur3" />
          <feFlood floodColor="#ffffff" floodOpacity="0.9" result="color3" /> {/* Um "core" branco/claro */}
          <feComposite in="color3" in2="blur3" operator="in" result="glow3" />

          {/* MESCLAR TODOS OS BRILHOS E A LINHA ORIGINAL */}
          <feMerge>
            <feMergeNode in="glow1"/>       {/* Brilho mais externo */}
            <feMergeNode in="glow2"/> 
            {/* <feMergeNode in="glow3"/>       */}
            <feMergeNode in="SourceGraphic"/> {/* A linha original sem brilho */}
          </feMerge>
        </filter>
      </defs>
